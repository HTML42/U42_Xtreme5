...
## 11) Email class
- Klasse im Framework: `latest/php/classes/email.class.php`
- Unterstützt UTF-8-konforme Header, HTML/Plaintext-Mails und Anhänge.
- Constructor-Konfiguration:
  - `simulate_local_emails` (default `true`)
  - `simulated_emails_directory` (default `_emails`)
  - `default_from_email`, `default_from_name`
- Bei Localhost (`localhost`, `127.0.0.1`, `::1`) schreibt die Klasse statt SMTP-Versand eine Datei nach `<PROJECT_ROOT>/_emails/*.txt`.
- Für Serienversand: `sendBulk($mails, $baseDelayMs, $delayGrowthFactor, $maxDelayMs)` mit ansteigendem Versandabstand.
...
- Assets are loaded globally from `css/xtreme/xtabs.less` and `js/classes/xtabs.class.js`.
